...
!function ($) {
    ...
    Gridded.prototype = {
        ...
        placeItem: function ($item) {
            ...
            $item
                .attr("data-" + this.options.widthAttr, iw)
                .attr("data-" + this.options.heightAttr, ih);

            // Loop and till we place the element or reach the row limit.
            while (placed === false && (!this.options.row || row <= this.options.row)) {
                for (var c = 0; c <= this.options.col - 1 ; c++) {
                    ...
                    if(empty == true) {
                        ...
                        $item
                            .attr("data-pr", c)
                            .attr("data-pd", row)
                            .attr("data-itemCount", this.placesItemCount)
                            .css({
                                "position": "absolute",
                                "width": itemWidth,
                                "height": itemHeight,
